...
- No page calls `Math.random()`. Every random draw goes through a per-file `rnd()` helper (plus `rndInt(n)` for array indices, and each game's own `rand(a, b)` range helper) backed by `crypto.getRandomValues`, buffered 256 values at a time. None of these draws is security-relevant — they time enemy waves and scatter particles — but `Math.random()` is a non-cryptographic PRNG that static analysis (CodeQL `js/insecure-randomness`) flags wherever it appears, so each file keeps one audited source of randomness. Keep this pattern when adding a page.
