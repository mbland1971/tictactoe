...
Common structure across the canvas-based games (`centipede.html`, `asteroids.html`, `space-invaders.html`):
...
- High scores persisted per-game to `localStorage` under per-game keys (`centipede_hi`, `asteroids_hi`, `sinvaders_hi`), which `index.html` also reads to show high scores on the launcher cards.
